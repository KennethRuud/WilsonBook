...
%
  \node[color=black, rectangle, draw, text badly centered, rounded corners, %
       ...
       (HostDriver) {Host program's driver to call OpenRSP};
%
  \node[color=red, fill=red!10, rectangle, draw, text badly centered, rounded corners, %
        minimum height=20, text width=70, below of=OpenRSP-C-adapters, node distance=70, %
        xshift=-150, yshift=0] (Perturbations) {Perturbations};
  \node[color=red, fill=red!10, rectangle, draw, text badly centered, rounded corners, %
        minimum height=20, text width=45, below of=OpenRSP-C-adapters, node distance=70, %
        xshift=-100, yshift=-30] (Overlap) {Overlap operator};
  \node[color=red, fill=red!10, rectangle, draw, text badly centered, rounded corners, %
        minimum height=20, text width=60, below of=OpenRSP-C-adapters, node distance=70, %
  ...
  \node[color=red, fill=red!10, rectangle, draw, text badly centered, rounded corners, %
        minimum height=20, text width=75, below of=OpenRSP-C-adapters, node distance=70, %
  ...
  \node[color=red, text badly centered, minimum height=20, text width=100, %
       ...
       {\textbf{Host program's callback functions for OpenRSP}};
%
  \node[color=black, rectangle, draw, text badly centered, rounded corners, %
...
%    {OpenRSP APIs (C or Fortran)} (OpenRSP);
  \draw [-latex'] (OpenRSP-C-API) edge node[align=center, midway, yshift=15, text width=120] %
    {Calculate molecular response properties} (OpenRSP-response);
  \draw [-latex'] (OpenRSP-response) edge node[align=left, midway, xshift=77, text width=145] %
    {Get information of perturbations, contributions from Hamiltonian, %
     and response parameters} (OpenRSP-C-adapters);
  \draw [-latex'] (OpenRSP-C-API) -- %
    ...
    {Set information of perturbations, Hamiltonian, and linear response %
     equation solver} (OpenRSP-C-API |- OpenRSP-C-adapters) |- (OpenRSP-C-adapters);
  \draw [-latex'] (OpenRSP-library)--(QcMatrix);
  \draw [-latex'] (HostDriver)--(OpenRSP-C-API);
  \draw [-latex'] (OpenRSP-C-adapters)--(Perturbations);
  \draw [-latex'] (OpenRSP-C-adapters)--(Overlap);
  \draw [-latex'] (OpenRSP-C-adapters)--(OneOper);
  \draw [-latex'] (OpenRSP-C-adapters)--(TwoOper);
  \draw [-latex'] (OpenRSP-C-adapters)--(XCFun);
  \draw [-latex'] (OpenRSP-C-adapters)--(ZeroOper);
  \draw [-latex'] (OpenRSP-C-adapters)--(Solver);
